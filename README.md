...
- Inspect the `package.xml` of the package for any rosdoc2 specific configurations.
  ...
  - Note: the configuration consists of stanzas, each represents an instance of a 'builder' and takes the form of `'output subdir': {<settings>}`.
...
- Generate the documentation. For each builder specified (in order defined):
  ...
  - Attempt to move all files generated to their final destination in the output directory.
